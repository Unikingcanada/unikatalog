/**
 * TTCProductCard — Product card for Table Top Chain catalog
 * Compact, procurement-focused. No brand logos.
 */
    ...
    >
      ...
      >
        ...
        <div style={{ position: "absolute", top: 8, left: 8, display: "flex", flexDirection: "column", gap: 4 }}>
          <span style={{
            ...
            fontWeight: 800, padding: "2px 7px", borderRadius: 4, letterSpacing: "0.5px",
          }}>
            {isSteelCat ? "STEEL" : "PLASTIC"}
          </span>
          <span style={{
            background: "rgba(255,255,255,0.92)", color: accentColor, fontSize: 9,
            fontWeight: 700, padding: "2px 7px", borderRadius: 4, border: `1px solid ${accentColor}`,
          }}>
      ...
      <div style={{ padding: "12px 14px", flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div onClick={() => onViewDetails(product)}>
          <div style={{ fontSize: 15, fontWeight: 800, color: C.navyMid, marginBottom: 2 }}>{product.name}</div>
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            <span style={{
              fontSize: 9, fontWeight: 700, color: PITCH_COLORS[product.pitch] || C.muted,
              background: "#f0f4ff", padding: "2px 7px", borderRadius: 10,
            }}>
              {product.pitch}
            </span>
        ...
        {/* Description */}
        <p onClick={() => onViewDetails(product)} style={{ fontSize: 11, color: C.muted, lineHeight: 1.55, margin: 0, flex: 1 }}>
          {product.description}
        </p>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
          {product.applications.slice(0, 3).map((app, i) => (
            <span key={i} style={{
              fontSize: 9, color: C.navyMid, background: "#eef3f8",
        ...
        <div style={{
          ...
          <span style={{
            ...
            background: `${accentColor}15`, padding: "1px 8px", borderRadius: 8,
          }}>{matCount}</span>
        </div>
        ...
        {product.widths?.length > 0 && (
          <div style={{ fontSize: 10, color: C.muted }}>
            <span style={{ fontWeight: 600, color: C.navyMid }}>Widths: </span>
            {product.widths.slice(0, 4).join(", ")}
            {product.widths.length > 4 ? ` +${product.widths.length - 4} more` : ""}
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
          ...
          <button
            onClick={(e) => { e.stopPropagation(); onAddRFQ(product, null); }}
            style={{
