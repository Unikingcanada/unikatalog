...
function fuzzyScore(str, pattern) {
  ...
  if (str.includes(pattern)) return 1000 + (100 - str.length * 0.1);
  // Word-start match
  const words = str.split(/[\s\-_,/]+/);
  for (const w of words) {
    if (w.startsWith(pattern)) return 800 + (100 - str.length * 0.1);
  }
...
function searchStr(p) {
  return [
    p.series, p.type, p.style, p.category, p.notes, p.materials, p.material,
    p.application, p.brand, p.duty, p.discharge_type, p.part_number, p.description,
...
export default function HomeGlobalSearch({ allData = [], onSelect }) {
  ...
  const results = useMemo(() => {
    ...
    for (const p of allData) {
      ...
      if (allMatch) scored.push({ p, score: total });
    }
    return scored.sort((a, b) => b.score - a.score).slice(0, 12).map(s => s.p);
  }, [query, allData]);
  ...
  return (
    <div style={{ position: "relative", maxWidth: 560, margin: "0 auto" }}>
      <span style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", fontSize: 18, color: "#94a3b8", pointerEvents: "none", zIndex: 1 }}>🔍</span>
      <input
        ref={inputRef}
        value={query}
      ...
      {query && (
        <button
          ...
          style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "#94a3b8", lineHeight: 1, zIndex: 1 }}
        >×</button>
      )}
      ...
      {showDrop && (
        <div ref={dropRef} style={{
          ...
          background: "#fff", border: `1.5px solid ${C.navyMid}`, borderTop: "none",
          ...
          {results.length === 0 ? (
            ...
            <>
              <div style={{ padding: "8px 16px 6px", fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: "0.6px", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between" }}>
                <span>{results.length} result{results.length !== 1 ? "s" : ""}</span>
                <span style={{ fontWeight: 400 }}>{allData.length} products indexed</span>
              </div>
              <div style={{ maxHeight: 420, overflowY: "auto" }}>
                {results.map((p, i) => (
                  ...
                  >
                    {p.image_url ? (
                      <img src={p.image_url} alt="" style={{ width: 44, height: 36, objectFit: "contain", borderRadius: 6, background: C.bg, flexShrink: 0 }} onError={e => e.target.style.display = "none"} />
                    ) : (
                      <div style={{ width: 44, height: 36, borderRadius: 6, background: "#f1f5f9", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, color: "#cbd5e1" }}>⛓</div>
                    )}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: C.muted, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {[p.style !== p.series ? p.style : null, p.application, p.materials].filter(Boolean).slice(0, 2).join(" · ")}
                      </div>
                    </div>
