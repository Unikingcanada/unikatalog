...
const CATALOG_URL = "https://wpbackend.movexii.com/wp-content/uploads/2024/11/Movex_Conveyor_modular_belts_and_chains_Catalog_Digital_Imperial.pdf";

function TabButton({ label, active, onClick }) {
  return (
    <button onClick={onClick} style={{
      padding: "8px 14px", background: active ? C.navyMid : "transparent",
...
function StyleCard({ style, onViewSpecs, onConfigure }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ height: 130, overflow: "hidden", background: "#f0f4f8", position: "relative", flexShrink: 0 }}>
        {style.image ? (
          ...
          <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 4, color: C.muted }}>
            ...
            <div style={{ fontSize: 8, textAlign: "center", padding: "0 8px" }}>Image to be added by Uniking</div>
          </div>
        )}
      ...
      <div style={{ padding: "10px 12px", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 6, flexWrap: "wrap" }}>
          ...
          {style.surface && (
            <span style={{ background: "#f1f5f9", color: C.muted, fontSize: 9, padding: "1px 6px", borderRadius: 7 }}>{style.surface?.slice(0, 20)}</span>
          )}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5, marginTop: "auto" }}>
          ...
          <button onClick={() => onConfigure(style)} style={{ padding: "7px", background: C.navyMid, border: "none", borderRadius: 6, cursor: "pointer", fontSize: 10, fontWeight: 800, color: "#fff" }}>
            Configure →
          </button>
        </div>
      </div>
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", zIndex: 1000, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: 16, overflowY: "auto" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 680, boxShadow: "0 24px 80px rgba(0,0,0,0.3)", marginBottom: 16 }}>
        <div style={{ background: C.navyMid, borderRadius: "16px 16px 0 0", padding: "16px 22px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <div style={{ background: MOVEX_RED, borderRadius: 4, padding: "2px 7px" }}>
                <span style={{ fontSize: 9, fontWeight: 800, color: "#fff" }}>MOVEX</span>
              </div>
        ...
        <div style={{ padding: "18px 22px" }}>
          ...
          <div style={{ background: C.bg, borderRadius: 8, padding: "12px 14px", marginBottom: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 11 }}>
              {[
                ...
                ["Pitch", series.pitch_in && series.pitch_in !== "To be confirmed by Uniking" ? `${series.pitch_in}" (${series.pitch_mm} mm)` : "To be confirmed by Uniking"],
                ["Surface", style.surface],
                ["Open Area", style.openArea],
                ["Catalog Page", series.catalogPage ? `p. ${series.catalogPage}` : "—"],
                ["Min Width", series.min_width_mm ? `${series.min_width_in}" (${series.min_width_mm} mm)` : "To be confirmed by Uniking"],
                ["Width Increment", series.width_increment_mm ? `${series.width_increment_in}" (${series.width_increment_mm} mm)` : "To be confirmed by Uniking"],
                ["Backflex Radius", series.backflex_radius_mm ? `${series.backflex_radius_mm} mm (${series.backflex_radius_in}")` : "To be confirmed by Uniking"],
                ["Fixation", series.fixation || "To be confirmed by Uniking"],
                ["Collapse Factor", series.collapse_factor || "—"],
          ...
          {style.notes?.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              ...
              {style.notes.map((n, i) => <div key={i} style={{ fontSize: 11, color: C.text, marginBottom: 3, display: "flex", gap: 5 }}><span style={{ color: MOVEX_RED }}>•</span>{n}</div>)}
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={() => { onClose(); onConfigure(style); }} style={{ flex: 2, padding: "10px", background: C.navyMid, color: "#fff", border: "none", borderRadius: 9, cursor: "pointer", fontSize: 12, fontWeight: 800 }}>
              Configure This Belt →
            </button>
          </div>
        </div>
...
export default function MovexSeriesDetail({ series, onConfigure, onBack, onGoRFQ }) {
  ...
  function addToRFQ(item) {
    ...
    cart.push({
      cartId: `movex_${item.name?.replace(/\s/g, "_")}_${Date.now()}`,
      id: `movex_${Date.now()}`,
      _source: "movex_component",
      series: `${series.name} — ${item.name}`,
      name: item.name,
      type: item.type || "Movex Component",
      style: series.name,
  ...
  const accessoryTypes = [...new Set((series.accessories || []).map(a => a.type))];
    ...
    <div>
      ...
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyMid})`, padding: "20px 24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
          <div style={{ background: MOVEX_RED, borderRadius: 5, padding: "3px 8px" }}>
            <span style={{ fontSize: 10, fontWeight: 800, color: "#fff" }}>MOVEX</span>
          </div>
          {series.name.includes("Blueline") && (
            <div style={{ background: "#1e40af", borderRadius: 5, padding: "3px 8px" }}>
              <span style={{ fontSize: 10, fontWeight: 800, color: "#fff" }}>Blueline®</span>
            </div>
          )}
          <span style={{ color: "rgba(255,255,255,0.6)", fontSize: 12 }}>Modular Belting</span>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          {series.min_width_in && typeof series.min_width_in === "number" && <span style={{ background: "rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.8)", padding: "4px 10px", borderRadius: 16, fontSize: 11 }}>Min width: {series.min_width_in}"</span>}
          {series.fixation && series.fixation !== "Missing Data – Needs Mapping" && <span style={{ background: "rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.8)", padding: "4px 10px", borderRadius: 16, fontSize: 11 }}>{series.fixation}</span>}
      ...
      <div style={{ padding: "20px", background: C.bg, minHeight: 400 }}>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>{series.styles?.length} catalog styles · Click <strong>View Details</strong> for specs or <strong>Configure</strong> to build your belt.</div>
              </div>
              <a href="https://www.movexii.com/en/prodcat/modular-belts" target="_blank" rel="noreferrer"
                style={{ fontSize: 11, color: MOVEX_RED, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" }}>
                View on Movex ↗
              </a>
            </div>
        ...
        {activeTab === "sprockets" && (
          <div>
            ...
            <div style={{ fontSize: 12, color: C.muted, marginBottom: 12 }}>Sprockets compatible with {series.name}. Click "+ RFQ" to add to your quote.</div>
            {series.sprockets?.length > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {series.sprockets.map((sp, i) => (
                  <div key={i} style={{ background: "#fff", borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden", display: "flex", alignItems: "center", gap: 12, padding: "12px 16px" }}>
                    <div style={{ width: 48, height: 40, flexShrink: 0, background: "#f0f4f8", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>⚙️</div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {sp.notes && <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{sp.notes.slice(0, 80)}</div>}
                    </div>
                    <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                      {sp.url && <a href={sp.url} target="_blank" rel="noreferrer" style={{ color: MOVEX_RED, fontSize: 11, fontWeight: 700, textDecoration: "none" }}>Movex ↗</a>}
            ...
            <div style={{ marginTop: 12, fontSize: 11, color: C.muted }}>
              Full sprocket catalog: <a href="https://www.movexii.com/en/prodcat/sprockets-turning-disks" target="_blank" rel="noreferrer" style={{ color: MOVEX_RED }}>Movex Sprockets & Turning Disks ↗</a>. Contact Uniking for detailed sprocket selection.
            </div>
          </div>
        )}
        ...
        {activeTab === "accessories" && (
          <div>
            ...
            {series.accessories?.length > 0 ? (
                ...
                return (
                  <div key={type} style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "capitalize", letterSpacing: "0.5px", marginBottom: 8 }}>
                      {type === "nosebar" ? "Nosebars" : type === "flight" ? "Flights" : type === "sideguard" ? "Sideguards" : type === "track" ? "Tracks" : type === "wearstrip" ? "Wearstrips" : "Accessories"} ({items.length})
                    </div>
                      ...
                      {items.map((acc, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderBottom: i < items.length - 1 ? `1px solid ${C.border}` : "none" }}>
                          <div style={{ width: 40, height: 36, flexShrink: 0, background: "#f1f5f9", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>
                            {acc.type === "flight" ? "🔧" : acc.type === "nosebar" ? "🔩" : acc.type === "sideguard" ? "🛡️" : acc.type === "track" ? "🛤️" : "📦"}
                          </div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {acc.url && <a href={acc.url} target="_blank" rel="noreferrer" style={{ fontSize: 10, color: MOVEX_RED, fontWeight: 700, textDecoration: "none", flexShrink: 0 }}>Movex ↗</a>}
              ...
              <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 8, padding: "14px", fontSize: 13, color: "#92400e" }}>
                ⚠ Accessories for {series.name}: <strong>To be confirmed by Uniking.</strong>
              </div>
            )}
          </div>
        )}
            ...
            <div style={{ background: "#fff", borderRadius: 10, border: `1px solid ${C.border}`, padding: "16px 18px", marginBottom: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 10 }}>
                {[
                  ...
                  ["Pitch", series.pitch_in && series.pitch_in !== "To be confirmed by Uniking" ? `${series.pitch_in}" (${series.pitch_mm} mm)` : "To be confirmed by Uniking"],
                  ["Catalog Page", series.catalogPage ? `p. ${series.catalogPage}` : "—"],
                  ["Min Width", series.min_width_mm && typeof series.min_width_mm === "number" ? `${series.min_width_in}" (${series.min_width_mm} mm)` : "To be confirmed by Uniking"],
                  ["Width Increment", series.width_increment_mm ? `${series.width_increment_in}" (${series.width_increment_mm} mm)` : "To be confirmed by Uniking"],
                  ["Backflex Radius", series.backflex_radius_mm ? `${series.backflex_radius_mm} mm` : "To be confirmed by Uniking"],
                  ["Pin Diameter", series.pin_dia_mm ? `${series.pin_dia_mm} mm (${series.pin_dia_in}")` : "To be confirmed by Uniking"],
                  ["Fixation Type", series.fixation || "To be confirmed by Uniking"],
                  ["Collapse Factor", series.collapse_factor || "—"],
                  ["Width Increment Note", series.width_increment_note || "—"],
            ...
            <div style={{ background: "#fff", borderRadius: 10, border: `1px solid ${C.border}`, padding: "14px 18px" }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <a href={CATALOG_URL} target="_blank" rel="noreferrer"
                  style={{ background: MOVEX_RED, color: "#fff", padding: "8px 16px", borderRadius: 8, fontWeight: 700, fontSize: 11, textDecoration: "none" }}>
                  📄 Movex Imperial Catalog (p.{series.catalogPage})
                </a>
                <a href="https://www.movexii.com/en/prodcat/modular-belts" target="_blank" rel="noreferrer"
              ...
              <div style={{ marginTop: 8, fontSize: 10, color: C.muted }}>
                Source: Movex Conveyor Modular Belts & Chains Catalog (Imperial, 2024). All specs subject to confirmation by Uniking.
              </div>
            </div>
          </div>
        )}
