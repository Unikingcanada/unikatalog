...
function Header({ step, onClose }) {
  return (
    <div style={{ background: `linear-gradient(90deg, ${C.navy}, ${C.navyMid})`, padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
    ...
    <div style={{ background: "#fff", padding: "10px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 4 }}>
      {STEP_LABELS.map((label, i) => (
        <div key={i} style={{ flex: 1, textAlign: "center" }}>
          <div style={{ height: 4, background: i <= step ? C.navyMid : "#e2e8f0", borderRadius: 4, marginBottom: 4 }} />
          <div style={{ fontSize: 9, color: i <= step ? C.navyMid : C.muted, fontWeight: i === step ? 700 : 400 }}>{label}</div>
        </div>
      ))}
    </div>
  );
    ...
    return (
      <div>
        ...
        <div style={body}>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 14 }}>Select the specific surface style for your application.</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 10 }}>
