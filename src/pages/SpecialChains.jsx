...
const CHAIN_CATEGORIES = [
  {
    ...
    // Load all products with category = "Pintle Chain"
    fetchMode: "category",
  },
  {
    key: "Long Link Chain",
  ...
  },
  {
    ...
    fetchMode: "category",
    subcategories: [
      { key: "DLI Scanner Chain", label: "DLI Scanner Chain", desc: "High-speed chains for DLI mail and parcel sorting systems." },
      { key: "Double Flex Chain", label: "Double Flex Chain (DF3500)", desc: "Flexible in multiple directions — dual pitch, 48,000 lbs." },
      { key: "Waste Water Chain", label: "Waste Water Chain", desc: "WR720S & WR730S for municipal waste water conveyors." },
      { key: "Paver Chain", label: "Paver Chain (SS40SL)", desc: "Road construction — 3.075\" pitch, 60,000 lbs." },
    ],
  },
];

// ---- TEAR SHEET ----
function printTearSheet(p) {
...
<div class="header">
  ...
  <div class="header-text">
    ...
    <div class="header-type">${p.category}</div>
  </div>
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, paddingBottom: 16, borderBottom: `1px solid ${UNIKING_BORDER}` }}>
        ...
        <button onClick={() => printTearSheet(p)}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 40 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 28, fontWeight: 800, marginBottom: 12 }}>{p.part_number}</div>
          {p.industry && (
...
// ---- PRODUCT LIST (flat card grid + optional table summary) ----
function ProductList({ catKey, catInfo, onBack }) {
  ...
  useEffect(() => {
    setLoading(true);
    MacChainProduct.filter({ category: catKey })
  ...
  return (
    <div style={{ paddingTop: 28 }}>
      ...
      <div style={{ marginBottom: 28, paddingBottom: 16, borderBottom: `1px solid ${UNIKING_BORDER}` }}>
        <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 6 }}>{catInfo.label}</div>
        <div style={{ fontSize: 14, color: "#666", lineHeight: 1.6, maxWidth: 700 }}>{catInfo.desc}</div>
        {!loading && products.length > 0 && (
          <div style={{ fontSize: 12, color: "#999", marginTop: 8 }}>{products.length} product{products.length !== 1 ? "s" : ""} — click a card for full specifications</div>
        )}
      </div>
          ...
          {products.map(p => (
            <div key={p.id}
              ...
              <div style={{ padding: "12px 14px" }}>
                ...
                {p.subcategory && <div style={{ fontSize: 12, color: "#777", marginBottom: 2 }}>{p.subcategory}</div>}
                {p.industry && <div style={{ fontSize: 11, color: "#aaa", lineHeight: 1.4 }}>{p.industry}</div>}
              </div>
            </div>
          ))}
        </div>
...
// ---- SPECIAL APPLICATION — subcategory nav then product list ----
function SpecialAppView({ onBack }) {
  const cat = CHAIN_CATEGORIES.find(c => c.key === "Special Application Chain");
  const [selectedSubcat, setSelectedSubcat] = useState(null);

  if (selectedSubcat) {
    return <SubcatProductList catKey="Special Application Chain" subcatKey={selectedSubcat} subcatLabel={cat.subcategories.find(s=>s.key===selectedSubcat)?.label} onBack={() => setSelectedSubcat(null)} />;
  }
  ...
  return (
    <div style={{ paddingTop: 28 }}>
      ...
      <div style={{ marginBottom: 32, paddingBottom: 16, borderBottom: `1px solid ${UNIKING_BORDER}` }}>
        <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>{cat.label}</div>
        <div style={{ fontSize: 14, color: "#666", lineHeight: 1.7, maxWidth: 700 }}>{cat.desc}</div>
      </div>
...
function SubcatProductList({ catKey, subcatKey, subcatLabel, onBack }) {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState(null);
  ...
  useEffect(() => {
    ...
    MacChainProduct.filter({ category: catKey })
      .then(data => {
        // Match by subcategory or by part_number containing the key
        const filtered = data.filter(p => {
          if (subcatKey === "DLI Scanner Chain") return ["mrb124-narrow","mrb124-wide","mrbc100","mrbc124","mrb2512"].includes(p.slug);
          if (subcatKey === "Double Flex Chain") return p.slug === "df3500-double-flex-chain";
          if (subcatKey === "Waste Water Chain") return p.slug === "welded-steel-waste-water-chain";
          if (subcatKey === "Paver Chain") return p.slug === "paver-chain";
          return p.subcategory === subcatKey;
        });
  ...
  return (
    <div style={{ paddingTop: 28 }}>
      <button onClick={onBack} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, color: "#666", marginBottom: 20 }}>
        ← Back to Special Application Chain
      </button>
      <div style={{ marginBottom: 28, paddingBottom: 16, borderBottom: `1px solid ${UNIKING_BORDER}` }}>
        <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 6 }}>{subcatLabel}</div>
        {!loading && products.length > 0 && (
          <div style={{ fontSize: 12, color: "#999", marginTop: 8 }}>{products.length} product{products.length !== 1 ? "s" : ""} — click a card for full specifications</div>
        )}
      </div>
...
export default function SpecialChains() {
  ...
  const catInfo = CHAIN_CATEGORIES.find(c => c.key === selectedCategory);

  const renderContent = () => {
    if (!selectedCategory) return null;

    if (selectedCategory === "Special Application Chain") {
      return <SpecialAppView onBack={() => setSelectedCategory(null)} />;
    }
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', Arial, sans-serif", minHeight: "100vh", background: "#fff", color: UNIKING_DARK }}>
      ...
      <div style={{ background: UNIKING_DARK, color: "#fff", padding: "18px 32px", display: "flex", alignItems: "center", gap: 16, borderBottom: `3px solid ${UNIKING_RED}` }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 20, fontWeight: 700 }}>
            {selectedCategory ? (catInfo?.label || selectedCategory) : "Steel Pintle, Long Link & Special Application Chain"}
          </div>
        </div>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px 48px" }}>
        {selectedCategory ? renderContent() : (
          /* Landing — chain type cards */
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24 }}>
              {CHAIN_CATEGORIES.map(cat => (
                <button key={cat.key} onClick={() => setSelectedCategory(cat.key)}
                  ...
                  <div style={{ fontSize: 13, color: "#666", lineHeight: 1.6 }}>{cat.desc}</div>
                </button>
              ))}
            </div>
          </div>
